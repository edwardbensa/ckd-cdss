...
num_features = ["age", "time", "egfr", "acr"]
...
X_train = preprocessor.fit_transform(X_train_raw) # type: ignore
...
train_df.to_csv(INTERIM_TABLES_DIR / 'kfre_train.csv', index=False)

test_df = pd.DataFrame(X_test, columns=feature_names)
test_df["esrd"] = y_test.values
test_df.to_csv(INTERIM_TABLES_DIR / 'kfre_test.csv', index=False)

# Save preprocessing pipeline
joblib.dump(preprocessor, MODELS_DIR / "kfre/preprocessor.joblib")
