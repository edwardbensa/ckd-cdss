...
from .utils import (
    select_device, evaluate_model, build_bounds_and_specs,
    sample_random_point, tensor_to_params, plot_gp_posterior
)

GP_PLOTS_DIR = INTERIM_DATA_DIR / "gp_plots"
...
def botorch_tuning(
    ...
    # Check if any hyperparameters available for tuning
    if len(specs) == 0:
        if verbose:
            logger.info(
                f"[{model_key}] No hyperparameters to tune. "
                f"Evaluating model with default parameters..."
            )

        # Evaluate once with empty params
        score = evaluate_model(X, y, model_key)
        if verbose:
            logger.success(f"[{model_key}] Score with default params: {score:.4f}")
        if final_tuning:
            return {}, score
        return score
    ...
    for t in pbar:
        ...
        # GP posterior plot
        if plot_gp:
            plot_gp_posterior(posterior, y_train, t, GP_PLOTS_DIR, final_tuning)
