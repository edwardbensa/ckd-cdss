...
from src.tabular.utils.train_boed.nn import BayesianNN, DeepBNN, SimpleCNN, Autoencoder, TorchWrapper
...
MODEL_REGISTRY = {
    ...
    "bnn": {
        ...
        "botorch_space": [
            ...
            {"name": "epochs",       "type": "int",   "low": 50,  "high": 150}, # Expanded range
    ...
    # Ensemble BNN
    "deep_bnn": {
        "model_fn": lambda params: TorchWrapper(DeepBNN, **params), # Fixed to **params
        "botorch_space": [
    ...
    # CNN (Tabular 1D)
    "cnn": {
        "model_fn": lambda params: TorchWrapper(SimpleCNN, **params), # Fixed to **params
        "botorch_space": [
            {"name": "num_filters", "type": "int",   "low": 16,  "high": 128},
            {"name": "kernel_size", "type": "int",   "low": 2,   "high": 5},
            {"name": "lr",          "type": "float", "low": 1e-4, "high": 1e-2, "log": True},
            {"name": "epochs",      "type": "int",   "low": 20,  "high": 100}, # Increased for CNN
            {"name": "batch_size",  "type": "int",   "low": 32,  "high": 128},
        ],
    },
    # Autoencoder
    "autoencoder": {
        "model_fn": lambda params: TorchWrapper(Autoencoder, **params), # Fixed to **params
        "botorch_space": [
            {"name": "hidden_dim_1",   "type": "int",   "low": 32,  "high": 256}, 
